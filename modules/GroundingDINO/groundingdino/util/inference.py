...
def _cxcywh_to_xyxy(boxes: torch.Tensor) -> np.ndarray:
    """Convert (cx, cy, w, h) -> (x1, y1, x2, y2) for a tensor of boxes."""
    cx, cy, w, h = boxes.unbind(-1)
    x1 = cx - 0.5 * w
    y1 = cy - 0.5 * h
    x2 = cx + 0.5 * w
    y2 = cy + 0.5 * h
    stacked = torch.stack([x1, y1, x2, y2], dim=-1)
    return stacked.numpy()

# Lightweight stand-in for supervision.Detections to avoid supervision dependency.


class Detections:
    def __init__(self, xyxy: np.ndarray, confidence: np.ndarray | None = None):
        self.xyxy = xyxy
...
def annotate(
    ...
    # Draw using PIL to avoid cv2 font issues.
    image = Image.fromarray(image_source).convert("RGB")
    draw = ImageDraw.Draw(image)
    for (x1, y1, x2, y2), label in zip(xyxy, labels):
        draw.rectangle([(x1, y1), (x2, y2)], outline="red", width=2)
        draw.text((x1 + 2, max(0, y1 - 12)), label, fill="red")
...
class Model:
    ...
    def post_process_result(
        ...
        return Detections(xyxy=xyxy, confidence=confidence)
