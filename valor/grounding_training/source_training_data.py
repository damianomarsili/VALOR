...
def _resolve_vsr_image(image_link: str | None) -> Path | None:
    """
    Download a VSR sample image link to a temporary file and return the path.
    """
    if not image_link:
        return None
...
def decode_base64_image(data: str) -> Image.Image:
    """
    Decode a base64-encoded image payload, returning a RGB PIL image.
    """
...
def write_merged_dataset_config(odvg_dir: Path, output_path: Path | None = None) -> None:
    """
    Create a merged dataset descriptor that enumerates every ODVG jsonl file.
    """
    entries = []
    label_map = (odvg_dir / "label_map.json").resolve()
    for jsonl_file in sorted(odvg_dir.glob("*.odvg.jsonl")):
        entries.append(
            {
                "root": "",
                "anno": str(jsonl_file.resolve()),
                "label_map": str(label_map),
                "dataset_mode": "odvg",
            }
        )

    payload = {"train": entries}
    output = output_path or (odvg_dir / "merged_dataset.json")
    output.write_text(json.dumps(payload, indent=2), encoding="utf-8")
    print(f"Wrote merged dataset config with {len(entries)} entries to {output}.")
...
def iter_gqa_samples(data_root: Path) -> Iterable[QuestionSample]:
    ...
    images = load_dataset("lmms-lab/GQA", "train_balanced_images", split="train").with_format("python")

    image_lookup = {}
    for img_sample in images:
        image = img_sample.get("image")
        image_id = img_sample.get("id")
        if image is None or image_id is None:
            continue
        image_lookup[str(image_id)] = image

    for sample in instructions:
        question = sample.get("question")
        image_id = sample.get("imageId")
        if not question or image_id is None:
            continue
        image = image_lookup.get(str(image_id))
        if image is None:
            continue
        yield QuestionSample(question=question, image=image, prefix="gqa", image_id=str(image_id))
...
def iter_vsr_samples(data_root: Path) -> Iterable[QuestionSample]:
    ...
    for row in dataset:
        ...
        resolved = _resolve_vsr_image(image_link)
        if resolved is None:
            continue
    ...
    for idx, sample in enumerate(tqdm(samples, desc=f"{name}")):
        ...
        try:
            ...
            for call_idx, call in enumerate(calls):
                ...
                save_record(record, (dataset_dir / "samples") / filename)
...
def main() -> None:
    ...
    if not os.environ.get("OPENAI_API_KEY"):
        raise RuntimeError(
            "OPENAI_API_KEY must be set in the environment before running this script."
        )
    ...
    write_merged_dataset_config(args.odvg_dir)
