...
class RLHFDataset(Dataset):
    ...
    def __init__(
        ...
        # Dami - Image configs
        self.max_pixels = config.get("max_pixels", 1080**2)
        ...
        # Dami - only loads one dataset at a time.
        data_path = self.data_files[0]
    ...
    def process_image(self, image: Union[Dict[str, Any], ImageObject]) -> ImageObject:
        ...
        if (
            self.max_side_length is not None
            and max(image.width, image.height) > self.max_side_length
        ):
            sf = self.max_side_length / max(image.width, image.height)
            image = fetch_image(
                {
                    "image": image,
                    "resized_width": int(image.width * sf),
                    "resized_height": int(image.height * sf),
                }
            )

        if (
            self.min_side_length is not None
            and min(image.width, image.height) < self.min_side_length
        ):
